...
runners.forEach(item => {
    fullName.unshift(item.first_name + " " + item.last_name)
});
...
// Problem 1 Contact information
let contactInfo = [];

runners.forEach(item => {
    contactInfo.unshift(`Name: ${item.first_name} ${item.last_name}, Company: ${item.company_name}, E-Mail: ${item.email}`);
});
console.log(contactInfo);

// Problem 2 Premier Doners

const premierDoners = runners.map(item =>{
    if(item.donation >= 100){
        return item.first_name + " " + item.last_name;
    }
});

console.log(premierDoners);
